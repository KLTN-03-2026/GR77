...
export function useCreateCampaign() {
    ...
    const validateField = (name: string, value: string, form?: HTMLFormElement) => {
        let err = '';
        ...
        if (name === 'description') {
            ...
            if (!value.trim()) err = 'Vui lòng nhập địa điểm';
            ...
            else if (startVal < todayAtZero) err = 'Ngày bắt đầu không thể ở quá khứ';
            ...
            if (!value) err = 'Vui lòng chọn ngày kết thúc';
            else {
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            if (file.size > 5 * 1024 * 1024) {
                setFieldErrors(prev => ({ ...prev, coverImage: 'Image size must be less than 5MB' }));
    ...
    const removeImage = () => {
        setImageFile(null);
        setImagePreview('');
        setFieldErrors(prev => ({ ...prev, coverImage: 'Vui lòng chọn ảnh đại diện' }));
    ...
    const handleGalleryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (files.length + galleryPreviews.length > 5) {
            setFieldErrors(prev => ({ ...prev, gallery: 'Maximum 5 gallery images allowed' }));
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const fieldsToValidate = [
            'title', 'description', 'locationText', 'fundingGoalAmount', 
            'minimumDonationAmount', 'startAt', 'endAt'
        ];

        fieldsToValidate.forEach(fieldName => {
            const val = formData.get(fieldName) as string;
            const err = validateField(fieldName, val, formObj);
            if (err) {
        ...
        if (!selectedCategoryId) {
            setFieldErrors(prev => ({ ...prev, categoryId: 'Vui lòng chọn danh mục' }));
            formHasError = true;
        }
